...
class ContentLoss(torch.nn.Module):
    ...
    def __init__(self) -> None:
        ...
        vgg19.load_state_dict(torch.load('./models/vgg19.pth'))

        # Extract the 36th layer of vgg19 model feature extraction layer.
        self.feature_extract = torch.nn.Sequential(*list(vgg19.features.children())[:35]).eval().cuda()
    ...
    def forward(self, source: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        loss = torch.nn.functional.l1_loss(self.feature_extract(source.float()), self.feature_extract(target.float()))
